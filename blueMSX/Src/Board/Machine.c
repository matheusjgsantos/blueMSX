...
static int readMachine(Machine* machine, const char* machineName, const char* file)
{
    ...
    if (configIni == NULL)
        return 0;
    fprintf(stdout,"DEBUG - Machine.c readMachine received %s as file\n", file);
    ...
    for (i = 0; i < sizeof(machine->slotInfo) / sizeof(SlotInfo) && *slotBuf; i++) {
        ...
        if (!machine->isZipped)
        {
                ...
                strcasestr(machine->slotInfo[i].name, "Machines\\") == machine->slotInfo[i].name)
            {
                ...
                fprintf(stdout,"DEBUG - Machine.c set expandedPath to %s",expandedPath);
...
Machine* machineCreate(const char* machineName)
{
    ...
    fprintf(stdout,"DEBUG - Machine.c machineCreate received %s and %s as machineDir and machineName\n",machinesDir,machineName);
...
void machineSetDirectory(const char* dir)
{
    ...
    fprintf(stdout,"DEBUG - Machine.c machineSetDirectory set %s from %s\n",machinesDir, dir);
}
